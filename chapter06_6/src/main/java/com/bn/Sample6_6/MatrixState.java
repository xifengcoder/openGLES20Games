...
public class MatrixState {
    ...
    public static void setLightDirection(float x, float y, float z) {
        ...
        //将定向光方向向量的X,Y,Z分量存入数组
        lightDirection[0] = x;
        lightDirection[1] = y;
        lightDirection[2] = z;
        llbbL.order(ByteOrder.nativeOrder());//设置字节顺序
        lightDirectionFB = llbbL.asFloatBuffer(); //转换为float类型数据缓冲
        lightDirectionFB.put(lightDirection); //将光源方向向量放入数据缓冲
        lightDirectionFB.position(0); //设置缓冲区起始位置
    }
}
