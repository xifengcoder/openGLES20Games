...
void directionalLight(                      //定向光光照计算的方法
    ...
    //计算从表面点到光源位置的向量vp
    //vec3 lightDirection = normalize(lightDirection - fragPos.xyz);
    vec3 lightDirection = normalize(lightDirection);  //规格化定向光方向向量
    ...
    float specularStrength = max(0.0, pow(dot(normalVector, halfVector), 50.0)); 	//镜面反射光强度因子
    lightSpecular = specularFactor * specularStrength;    			//计算镜面光的最终强度
}

void main(){
    vec4 lightAmbient, lightDiffuse, lightSpecular;	  //用来接收三个通道最终强度的变量
    directionalLight(lightAmbient, lightDiffuse, lightSpecular, uLightDirection,
